...
const GET_ONE_SHOTS = gql`
  query oneShotPaginateQuery(
    $first: Int
    $offset: Int
    $orderBy: [_One_ShotOrdering]
    $filter: _One_ShotFilter
  ) {
    One_Shot(first: $first, offset: $offset, orderBy: $orderBy, filter: $filter) {
      name
      dm {
        name
      }
...
function OneShotList(props: any) {
  const { classes } = props
  ...
  const { loading, data, error } = useQuery(GET_ONE_SHOTS, {
    variables: {
      first: rowsPerPage,
  ...
  return (
    <Paper className={classes.root}>
      <Title>One Shot List</Title>
      <TextField
        id="search"
        label="One Shot Name Contains"
              ...
              >
                <Tooltip title="Sort" placement="bottom-start" enterDelay={300}>
                  <TableSortLabel
                    ...
                    direction={order}
                    onClick={() => handleSortRequest('system')}
                  >
                    System
                  </TableSortLabel>
                </Tooltip>
...
export default withStyles(styles)(OneShotList)
